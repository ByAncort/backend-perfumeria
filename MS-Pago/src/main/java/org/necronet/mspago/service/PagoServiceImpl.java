...
public class PagoServiceImpl implements PagoService {
    ...
    public ServiceResult<Pago> procesarPago(Long carritoId, String metodoPagoStr) {
        try {
            ...
            // Validar que el carrito esté en estado adecuado para pagar
            ...
            // Simular datos de tarjeta (en producción usar servicio de tokens)
            pago.setUltimosCuatroDigitos("4242");
            pago.setNombreTitular("TITULAR EJEMPLO");

            // 3. Procesar pago (simulado - integrar con pasarela real)
            boolean pagoExitoso = simularProcesamientoPago();

            if (pagoExitoso) {
                pago.setEstado(EstadoPago.COMPLETADO);

                // 4. Confirmar carrito en el microservicio de carritos
    ...
    public ServiceResult<Pago> reembolsarPago(Long pagoId) {
        try {
            ...
            // Simular reembolso (en producción integrar con pasarela)
    ...
    private boolean simularProcesamientoReembolso() {
        // En producción, integrar con pasarela de pago
